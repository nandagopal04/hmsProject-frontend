...
const EditPatient = ({ patientId, onClose, onUpdate }) => {
  ...
  useEffect(() => {
    ...
    const fetchPatientData = async () => {
      try {
        const response = await axios.get(`http://hospitalmanagementsystem-env.eba-prkbihnf.ap-southeast-2.elasticbeanstalk.com/patient/${patientId}`);
        setPatientData(response.data);
      } catch (error) {
        console.error('Error fetching patient data for editing:', error);
      }
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://hospitalmanagementsystem-env.eba-prkbihnf.ap-southeast-2.elasticbeanstalk.com/patient/${patientId}`, patientData);
      onClose();
      onUpdate();
    } catch (error) {
      console.error('Error updating patient:', error);
